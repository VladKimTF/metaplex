...
const AdjustQuantitiesStep = ({
  ...
  const shouldRenderProbabilityInput =
  ...
  const handleDistributionChange = (
    ...
    if (inputType === InputType.probability) {
      return setPackState({
        probabilityByMetadataKey: {
          ...probabilityByMetadataKey,
          [pubKey]: number,
  ...
  return (
    <div className="quantities-step-wrapper">
      ...
      <div className="quantities-step-wrapper__table-titles">
        ...
        {shouldRenderProbabilityInput && <p>REDEEM PROBABILITY</p>}
      </div>
      ...
      {Object.values(selectedItems).map(item => (
        <ItemRow key={item.metadata.pubkey} item={item}>
          <>
            ...
            {shouldRenderProbabilityInput && (
              <div className="input-column">
                <Input
                  type="number"
                  min={0}
                  max={100}
                  value={probabilityByMetadataKey[item.metadata.pubkey]}
                  onChange={({ target: { value } }) =>
                    handleDistributionChange(
                      item.metadata.pubkey,
                      value,
                      InputType.probability,
                    )
                  }
                />
              </div>
